function [xkk,Pkk]=apivbkf_st_ghvg(xkk,Pkk,F,H,z,Q,R,f0,u0,e0,g0,v1,E_v,beta2_bar,sigama2,N)
...
%     xkk=xk1k+Kk*(z-zk1k);
    ...
    %%%%%%%Update auxiliary parameter  GGM(st)-GHSST
    %%%%%eta_1 and eta_3
    ...
    %%%%%Update kasai (Student's t)
    kasai=(nx*(1-E_t)+v1-2)/(eta_1_kasai+v1);
    
    %%%%%Update lamda (GH skew Student¡¯s t)
    lamda=(-(nz*(1-E_y)+E_v+2)+sqrt((nz*(1-E_y)+E_v+2)^2+4*eta_3_lamda*(eta_1_lamda+E_v)))/(2*eta_3_lamda);
