...
2. **imat()**：与mat类似，区别在于 iamt 创建的矩阵是**int**类型的。
...
3. **zeros()**：创建0矩阵、**double**类型。它比mat多了`\#if NOCALLOC if ((p=mat(n,m))) for (n=n*m-1;n>=0;n--) p[n]=0.0;`，如果预编译指令有NOCALLOC，会把所有位赋值0.0
...
8. **normv3()**：规格化三维向量（各元素除以向量的模长范数），a为传入向量，b为传出向量。
...
9. **matcpy()：**Copy matrix、将B矩阵的所有元素赋值给A矩阵 
...
10. **matmul()：**Multiply matrix、矩阵A、B相乘返回C
...
11. **matinv()：**Inverse of matrix、矩阵求逆
...
12. **solve()**：求线性方程组的解 、AX=Y，求 X，A 为 nn，Y为 nm，用的不多，好像只有模糊度固定里涉及到了。
...
13. **ludcmp ()**：LU分解，即把矩阵 A 分解 LU 乘积的形式，U 为单位上三角矩阵和 L 单位下三角矩阵两部分 。
...
14. **lubksb ()**：LU回代,即把单位上三角矩阵U和单位下三角矩阵L矩阵回代为一个整体矩阵 
