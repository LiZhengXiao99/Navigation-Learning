...
这个仓库会长期更新，做为我学习的记录，有人用得上，能给我带来一些成就感，激励着自己坚持学下去，就挺好。有些程序会详细写（比如 RTKLIB）；另外一些可能不会写的面面俱到（比如TGINS），就只画个流程图、列举重点的函数。列的开源代码很多，但其实目前只有 RTKLIB、GAMP、PSINS、GICI、KF-GINS、OB-GINS 内容稍微多一点：
