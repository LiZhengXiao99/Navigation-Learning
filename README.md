...
这个仓库会长期更新，做为我学习的记录，不求有啥回报，有人用得上，能给我带来一些成就感，激励着自己坚持学下去，就挺好了。有些程序会详细写（比如 RTKLIB）；另外一些可能不会写的面面俱到（比如TGINS），就只画个流程图、列举重点的函数。列的开源代码很多，但其实目前只有 RTKLIB、GAMP、PSINS、GICI、KF-GINS、OB-GINS 内容稍微多一点：

* **RTKLIB**：学的人多而且我用的也多，会写的很详细，仔细修改。
* **GAMP**：详细写。
* **KF-GINS**：已经写的很详细了，就一千五百行代码，学的很熟了。
* **OB-GINS**：也会写的很详细。
* **VINS** 和 **ORB-SLAM**：已经有很多高质量的教程，我就不继续更新了。
* **PPPLib**：这是我老师写的程序，之前整理过一个函数调用流程图，但以后大概率不继续看了。
* **TGINS**：也是我老师写的程序，最近一直在看，老师让我参照着写代码；用的人很少，笔记估计没别人会看，不会写太详细，就梳理一下函数调用关系、程序执行流程。
* **GICI-LIb**：之后有空就会更新。
* **GNSS-SDR**：打算学完 GNSS 接收机原理之后好好看看。
