...
% ------------------------------------------------------------------- 
function [PI,hatX,hatDP] = Riccati_MCCKF_SRCF_QL(matrices,initials_filter,measurements,handle_kernel)
...
function [X,sqrtP,residual] = sr_mcckf_update(X,sqrtP,z,H,sqrtR,lambda_k)
     [m,n]  = size(H);
   residual = z - H*X;   
  PreArray  = [inv(sqrtP'), sqrt(lambda_k)*H'/sqrtR';];
   [~,PostArray]  = qr(PreArray');
        PostArray = PostArray';
       sqrtPinv   = (PostArray(1:n,1:n))';

   Gain = lambda_k*eye(n)/sqrtPinv/sqrtPinv'*H'/sqrtR'/sqrtR;
      X = X + Gain * residual;
 
% the MCC-KF method implies Joseph stabilized formula for updating the error covariance matrix at the end

        PreArray  = [(eye(n) - Gain*H)*sqrtP, Gain*sqrtR;];
   [~,PostArray]  = qr(PreArray');
        PostArray = PostArray';
