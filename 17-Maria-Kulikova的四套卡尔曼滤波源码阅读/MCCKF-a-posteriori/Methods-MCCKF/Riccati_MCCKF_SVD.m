...
% ------------------------------------------------------------------- 
% References:
% 1. This is the implementation of Algorithm 1c from the following paper: 
...
function [PI,hatX,hatDP] = Riccati_MCCKF_SVD(matrices,initials_filter,measurements,handle_kernel)
...
function [X,DPsqrt,QP,residual] = svd_update(X,DPsqrt,QP,z,H,R_sqrt,Lk)
  ...
  residual = z - H*X;                       
     ...
     PreArray   = [sqrt(Lk)*eye(m)/R_sqrt'*H*QP; inv(DPsqrt);];
     [~,Sm,Vm]  = svd(PreArray);     
     DPsqrt_new = inv(Sm(1:n,1:end)); 
         QP_new = QP*Vm;

           Gain = Lk*(QP_new*(DPsqrt_new.^2)*QP_new')*H'/R_sqrt/R_sqrt'; 
              X = X + Gain*residual; 

    PreArray  = [DPsqrt*QP'*(eye(n) - Gain*H)'; R_sqrt*Gain';];
      [~,S,QP] = svd(PreArray);       
        DPsqrt = S(1:n,1:end);  
end
